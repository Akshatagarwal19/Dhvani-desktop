...
export function compareTracks(
  ...
  const artistSimilarity = compareTextSimilarity(
    trackA.artist,
    trackB.artist
  )
  ...
  if (artistSimilarity >= ARTIST_SIMILARITY) {
    ...
    reasons.push(
      `Artist ${Math.round(artistSimilarity)}% similar`
    )
  }

  const titleSimilarity = compareTextSimilarity(
    trackA.title,
    trackB.title
  )
  ...
  if (titleSimilarity >= TITLE_SIMILARITY) {
    ...
    reasons.push(
      `Title ${Math.round(titleSimilarity)}% similar`
    )
  }
  ...
  if (filenameSimilarity >= FILENAME_SIMILARITY) {
    ...
    reasons.push(
      `Filename ${Math.round(filenameSimilarity)}% similar`
    )
  }
  return {
    score,
